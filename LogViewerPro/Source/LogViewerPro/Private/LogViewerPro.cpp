...
void FLogViewerProModule::StartupModule()
{
	...
	MenuExtender->AddMenuExtension("DeveloperToolsDebug", EExtensionHook::After, PluginCommands, FMenuExtensionDelegate::CreateRaw(this, &FLogViewerProModule::AddMenuExtension));
			...
			SNew(SBox)
				...
				[
					SNew(STextBlock)
						...
						.Text(FText::FromString(TEXT("Log Viewer Pro v1.920\r\nby Dmitrii Labadin\r\nYour support matters!")))
				]
		];
...
TSharedRef<SDockTab> FLogViewerProModule::SpawnNamedLogViewerProTab(const FString& TabFileName)
{
	const FString TabNameNoRes = FPaths::GetBaseFilename(TabFileName);
	TSharedRef<SLogViewerProWidgetMain> Widget = SNew(SLogViewerProWidgetMain).LogOutputDevice(LogViewerProOutputDevice).Module(this);
	TSharedRef<SDockTab> Tab = SNew(SDockTab)
		.TabRole(ETabRole::DocumentTab)
		.Label(FText::FromString(TabNameNoRes))
		[
			Widget
		];
	Tab->SetOnTabClosed(SDockTab::FOnTabClosedCallback::CreateRaw(this, &FLogViewerProModule::HandleNamedTabClosed));
	Widget->SetParentTab(Tab);
	Widget->InitAsNamedTab(TabFileName);

	return Tab;
}

void FLogViewerProModule::HandleNamedTabClosed(TSharedRef<SDockTab> DockTab)
{
	IPlatformFile& PlatformFile = FPlatformFileManager::Get().GetPlatformFile();
	FString NamedTabsFolderPath = SLogViewerProSettingsButton::GetDefaultNamedTabsFolderPath();
	FString FileToRemovePath = FPaths::Combine(NamedTabsFolderPath, DockTab->GetTabLabel().ToString() + TEXT(".json"));

	IFileManager& FileManager = IFileManager::Get();
	const bool bRemoveResult = PlatformFile.DeleteFile(*FileToRemovePath);
	if (!bRemoveResult)
	{
		UE_LOG(LogTemp, Error, TEXT("LogViewer: Tried to remove named tab file %s, but for unknown reason, delete file operation was unssuccesful."), *FileToRemovePath);
	}
}

void FLogViewerProModule::OnOpenNewTabClicked(TSharedRef<SDockTab> DockTab, ETabActivationCause InActivationCause)
{
	TSharedRef<SDockTab> NewTab = SpawnEmptyLogViewerProTab();
	TabManager->InsertNewDocumentTab(TabNameOpenNewTab, FTabManager::ESearchPreference::PreferLiveTab, NewTab);
}

void FLogViewerProModule::RequestNewNamedTab(const FString& NamedTabFileName)
{
	TSharedRef<SDockTab> NamedTab = SpawnNamedLogViewerProTab(NamedTabFileName);
	TabManager->InsertNewDocumentTab(TabNameOpenNewTab, FTabManager::ESearchPreference::PreferLiveTab, NamedTab);
}

void FLogViewerProModule::SpawnNamedTabs()
{
	//if (!PlatformFile.DirectoryExists(*NamedTabsFolderPath))
	//{
	//	PlatformFile.CreateDirectory(*NamedTabsFolderPath);
	//}
	IPlatformFile& PlatformFile = FPlatformFileManager::Get().GetPlatformFile();
	FString NamedTabsFolderPath = SLogViewerProSettingsButton::GetDefaultNamedTabsFolderPath();
	FString SearchPath = FPaths::Combine(NamedTabsFolderPath, TEXT("*"));
