...
{    
    ...
    private bool ForceStaticLibInEditor = true;  // 强制使用静态库在编辑器中
    ...
    void ThirdParty(ReadOnlyTargetRules Target)
    {
        ...
        PublicIncludePaths.AddRange(new string[] { Path.Combine(ModuleDirectory, "..", "..", "ThirdParty", "v8" + v8LibSuffix, "Inc", "libplatform") });
        ...
        else if (Target.Platform == UnrealTargetPlatform.Mac)
        {
            ...
            else
            {
                ...
                // 注意：即使是 arm64 Mac，如果用 Rosetta 2 编译，库文件仍然是 x86_64
                // 所以这里不添加 _arm64 后缀，直接使用 macOSdylib
